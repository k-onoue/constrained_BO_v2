...
# argparseの設定
parser = argparse.ArgumentParser(description='Run Tensor Ring ALS decomposition benchmark with specified logical cores and rank.')
...
# ログファイルの設定
logfile_name = f"benchmark_log_cores_{logical_cores}_rank{rank_index}_tr_als"
...
# テンソルリングALS分解（TR ALS）の実行と再構成
def perform_tr_als_and_reconstruct(tensor, rank=1):
    start_time = time.time()

    # Tensor Ring ALS 分解
    factors = tensor_ring_als(tensor, rank=rank)
...
for shape in shapes:
    ...
    logging.info(f"Running Tensor Ring ALS decomposition for tensor shape {shape} with {logical_cores} logical cores and rank {selected_rank}")
    ...
    for trial in range(num_trials):
        ...
        # 分解と再構成の実行
        elapsed_time, reconstruction_error = perform_tr_als_and_reconstruct(tensor, rank=selected_rank)
